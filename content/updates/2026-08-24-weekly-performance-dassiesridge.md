...
The Sat/Sun red band across all 14 rows is the standout: a perfectly uniform, simultaneous, complete drop to zero across the entire fleet. A follow-up investigation via the Unity API ruled out a monitoring outage — data quality read 100% and heartbeat signals stayed healthy throughout, meaning the fleet was genuinely reporting zero, not going dark. The EMS dispatch schedule was actively commanding charging through the window (down to -8,237 kW at peak) but measured fleet output never moved off baseline (~-40 kW) — a dispatch-to-execution failure affecting all 14 units simultaneously, not 14 independent mechanical faults. Root cause sits between EMS dispatch calculation and PCS setpoint execution and remains open; escalated to the EMS/VPPOC vendor. Separately, Thursday's fleet-wide dip to ~78% (the same shape as the recurring Tuesday pattern from prior weeks) looks like routine overnight-discharge/morning-recharge cycling rather than a fault — a spot-check of a comparable weekday dip found it fully explained by legitimate low-SOC discharge activity, unrelated to the weekend event. PCS11 remains the weakest unit on every normal day this week, though the gap to the fleet narrowed since Thursday's dip pulled everyone else down closer to its usual level.
...
**Resolved** — confirmed as a real, fleet-wide event, not a monitoring outage. All 14 PCS units were verified individually via direct Unity API queries: SOC and availability flat at 0.0% for the full 22-23 Aug window (289 consecutive 10-minute readings per unit, zero variation), with 100% data quality and a healthy, incrementing heartbeat throughout — ruling out a telemetry gap. Over the same period, the EMS dispatch schedule was actively commanding charging (down to -8,237 kW at peak) but actual measured fleet output never moved from its baseline (~-40 kW). No declaration or override was applied to Availability Declared BESS (stayed at 100% throughout, confirming nothing was planned) or its Effective value (calculated, not manually entered) — this was a genuine, unplanned, system-detected outage, not an administrative action. Root cause sits between EMS dispatch calculation and PCS setpoint execution; the BESS Controller segment exposes no diagnostic signals of its own via the API, so this can't be narrowed further from Unity data alone. Escalated to the EMS/VPPOC vendor (Power Factors) for controller-side log review. Full investigation writeup: `PCS_Fleet_Investigation_22-23Aug2026.docx`.
